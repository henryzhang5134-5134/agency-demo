...
type Clip='arrival'|'idle-a'|'idle-b'|'smile'|'praise';
export class VideoDirector {
 ...
 constructor(private retry:HTMLButtonElement,private random:()=>number=Math.random){
  ...
  this.videos.forEach((v,i)=>{v.muted=true;v.pause();v.addEventListener('ended',()=>{if(i===this.front&&this.phase==='playing'){this.phase='waiting';this.wait=this.praise?.25:5+Math.random()*5;this.warm(this.praise?'praise':this.nextIdle);}});});
 ...
 requestPraise(){if(this.clock-this.requestedAt<10)return;this.requestedAt=this.clock;this.praise=true;if(this.phase==='waiting'){this.wait=Math.min(this.wait,.4);this.warm('praise');}}
 setPaused(value:boolean){this.paused=value;for(const v of this.videos){if(value)v.pause();}if(!value&&this.phase==='playing')void this.play(this.videos[this.front]);}
 tick(dt:number){if(this.paused)return;this.clock+=dt;if(this.phase==='waiting'){this.wait-=dt;if(this.wait<=0){const clip=this.praise?'praise':this.nextIdle;this.warm(clip);const next=this.videos[1-this.front];if(next.readyState<2)return;this.phase='transition';next.currentTime=0;void this.transition(clip);}}}
 private async transition(clip:Clip){
  const old=this.videos[this.front],next=this.videos[1-this.front];
  // Decode/play the incoming frame before exposing it. The old tail remains underneath.
  try{await next.play();}catch{this.phase='waiting';this.wait=2;this.retry.hidden=false;return;}
  ...
  await new Promise<void>(r=>setTimeout(r,180));old.pause();old.style.opacity='0';old.style.zIndex='0';this.front=1-this.front;this.active=clip;
  if(clip==='praise')this.praise=false;else this.nextIdle=this.chooseIdle(clip);
  this.phase='playing';this.retry.hidden=true;if(this.paused)next.pause();
 }
 report(){return{clip:this.active,phase:this.phase,wait:Math.max(0,Math.round(this.wait*10)/10),paused:this.paused,praisePending:this.praise,time:this.videos[this.front].currentTime};}
}

const stories=[
 ['第一次一起来看这家店，就让你帮忙找东西，怪不好意思的。','没关系，我也想看看这里。'],
 ['这家店的主人不常露面，倒是很在意来访的人。','听起来有点神秘。'],
 ['委托书刚才还在包里……也许混进旅行箱了。','我来慢慢整理找找。'],
...
export class ChatDirector {
 ...
 pair(text:string,reply:string,priority=false){if(this.storyMode||(!priority&&this.time<this.cooldown))return;if(this.reply){this.message(this.reply.text,true);this.reply=null;}this.message(text);this.reply={text:reply,at:this.time+1.3+Math.random()*.8};this.cooldown=this.time+8;this.next=Math.max(this.next,this.time+13);}
 ...
 tick(dt:number){this.time+=dt;if(this.storyMode)return;
