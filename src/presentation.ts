...
export class VideoDirector {
 ...
 private configure(v:HTMLVideoElement){v.defaultMuted=true;v.muted=true;v.volume=0;v.playsInline=true;for(const name of ['muted','playsinline','webkit-playsinline','x5-playsinline'])v.setAttribute(name,'');v.setAttribute('x5-video-player-type','h5-page');v.setAttribute('x5-video-player-fullscreen','false');}
 ...
 private warm(clip:Clip){const v=this.videos[1-this.front],src=`${assetBase}video/mobile/${clip}.mp4`;if(v.getAttribute('src')!==src){v.autoplay=false;v.pause();v.src=src;v.preload='auto';v.load();}}
 private async play(v:HTMLVideoElement){
  if(this.paused||document.hidden||this.playPending.has(v))return false;
  const generation=this.generation;this.configure(v);v.autoplay=true;this.retryAt=this.clock+1.2;this.playPending.add(v);
 ...
 private fallbackArrival(){if(this.arrivalNotified||this.finale)return;this.generation++;const v=this.videos[this.front];v.autoplay=false;v.pause();v.removeAttribute('src');v.poster=assetBase+'video/rest-poster.jpg';v.load();this.phase='waiting';this.wait=5;this.warm(this.nextIdle);this.finishArrival();}
 ...
 holdForCompletion(){this.generation++;this.finale=true;this.praise=false;this.phase='completion-hold';this.videos.forEach((v,i)=>{v.autoplay=false;v.pause();v.style.opacity=i===this.front?'1':'0';v.style.zIndex=i===this.front?'1':'0';});const pose=this.videos[this.front];pose.poster=assetBase+'video/completion-poster.jpg';pose.removeAttribute('src');pose.load();this.warm('completion');}
 ...
 tick(dt:number){if(this.paused)return;this.clock+=dt;
  ...
  if(!this.finale&&this.active==='arrival'&&this.phase==='playing'&&!this.arrivalNotified){this.arrivalElapsed+=dt;if(this.arrivalElapsed>=20){this.fallbackArrival();return;}}
  if(this.finale){
 ...
 report(){return{clip:this.active,phase:this.phase,wait:Math.max(0,Math.round(this.wait*10)/10),paused:this.paused,praisePending:this.praise,time:this.videos[this.front].currentTime,duration:this.videos[this.front].duration,completion:this.finale,playbackError:this.lastPlayError};}
}
